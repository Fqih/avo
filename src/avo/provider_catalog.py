...
def provider_options() -> tuple[ProviderOption, ...]:
    return (
        ...
        ProviderOption(
            key="claude-web",
            label="Claude Web (free/paid)",
            group="web",
            auth_kind="browser_only",
            default_model="claude-web",
            description="Open Claude in the browser; this does not configure Claude Code.",
            credential_key=None,
            access_note="Web login is separate from Claude Code and is not an Avo inference route.",
        ),
        ProviderOption(
            key="claude-code",
            label="Claude Code",
            ...
            auth_kind="vendor_account",
            default_model="claude-sonnet-4-6",
            description="Use the official Claude Code OAuth/API route.",
            credential_key="claude",
            access_note="Claude Code requires an eligible plan or Anthropic API key.",
        ),
        ProviderOption(
            key="codex",
            label="Codex account",
        ...
        ProviderOption(
            key="chatgpt-web",
            label="ChatGPT Web (free/paid)",
            group="web",
            auth_kind="browser_only",
            default_model="chatgpt-web",
            description="Open ChatGPT in the browser; this does not configure Codex.",
            credential_key=None,
            access_note="Web login is separate from Codex and is not an Avo inference route.",
        ),
        ProviderOption(
            key="gemini-cli",
